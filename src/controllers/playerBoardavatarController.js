...
export const listAvatars = asyncHandler(async (req, res) => {
    ...
    const [items, total] = await Promise.all([
        PlayerBoardAvatar.find(q).sort({ createdAt: -1 }).skip(skip).limit(Number(limit)),
        PlayerBoardAvatar.countDocuments(q)
    ]);
...
export const updateAvatar = asyncHandler(async (req, res) => {
    const doc = await PlayerBoardAvatar.findById(req.params.id);
    if (!doc) {
        res.status(404);
        throw new Error("Avatar not found");
    }
...
export const addPurchaser = asyncHandler(async (req, res) => {
    ...
    const [avatar, user] = await Promise.all([
        PlayerBoardAvatar.findById(req.params.id),
    ...
    console.log(PlayerBoardAvatar.PurchaseBy);
    const purchaseBy = PlayerBoardAvatar.PurchaseBy || []; // Default to an empty array if undefined
    ...
    if (!exists) {
        ...
        PlayerBoardAvatar.PurchaseBy = purchaseBy; // Update the avatar object with the new PurchaseBy array
        await avatar.save();
    }
